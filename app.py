...
try:
    from config import (
        ...
        TRADE_PROFILES, DEFAULT_PROFILE,
    )
    from utils.timeutils import (
        now_utc, now_wib,
...
def main() -> None:
    ...
    # --- Trade Profile selector (profil bobot + gating per horizon trade) ---
    with st.sidebar:
        st.markdown("### 📐 Tipe Trade")
        _ptypes = list(TRADE_PROFILES.keys())
        _sel = st.selectbox(
            "Tipe Trade",
            _ptypes,
            index=_ptypes.index(DEFAULT_PROFILE),
            format_func=lambda k: TRADE_PROFILES[k]["label"],
            key="trade_profile_sel",
            help=(
                "Profil bobot + gating per horizon trade. Mempengaruhi: "
                "bobot R_hard/C/D/F, carry deadband, COT extreme, retail extreme, "
                "dan impact-minimum / freshness faktor F. "
                "Crypto selalu pakai bobot sendiri (tidak terpengaruh profil). "
                "⚠ Semua nilai = PLACEHOLDER — validasi via backtest."
            ),
        )
        active_profile = TRADE_PROFILES[_sel]
        st.caption(
            f"Bobot: R={active_profile['weights']['R_hard']} "
            f"C={active_profile['weights']['C']} "
            f"D={active_profile['weights']['D']} "
            f"F={active_profile['weights']['F']}"
        )
    ...
    try:
        ...
        # Ambil param surprise dari profil aktif (impact gate + freshness tau)
        _surp_impact_min = active_profile["surprise"]["impact_min"]
        _surp_tau        = float(active_profile["surprise"]["freshness_tau"])
        if st.session_state.get("pb_ff_data"):
            from datetime import datetime, timezone, timedelta
            _wib_now = datetime.now(timezone(timedelta(hours=7)))
            ff_scores = compute_ff_surprise(
                st.session_state["pb_ff_data"], _wib_now,
                impact_min=_surp_impact_min,
                tau=_surp_tau,
            )
        if not ff_scores:
            ff_scores = compute_ff_surprise_from_calendar(
                released_events,
                impact_min=_surp_impact_min,
                tau=_surp_tau,
            )
